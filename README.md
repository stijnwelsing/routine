...
- **Koers** — thema’s, weekoverzicht (hits/misses), A, B, nu, etappe, venster, trend, rem, volgende actie. Etappe schuift alleen als je zelf tikt; gear-down blokkeert omhoog. **Ik** is een blok op Koers, niet op Vandaag of Profiel.
...
Op Koers en Profiel: **Exporteer JSON** (v6-snapshot: profiel/doelen/thema’s + items + vector + etappe + events inclusief `body_weight`, `body_wake` en `body_meal`). Read-only.
