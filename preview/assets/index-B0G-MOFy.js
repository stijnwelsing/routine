...
<svg xmlns="http://www.w3.org/2000/svg" class="sprite" aria-hidden="true">
  ...
  <symbol id="i-dot-now" viewBox="0 0 24 24">
...
</svg>`;function ut(){document.querySelector(".sprite")||document.body.insertAdjacentHTML("afterbegin",ct)}const J=()=>document.querySelector("#app"),d={screen:"vandaag",skipItemId:null,advanceWarn:!1,busy:!1,error:null};let k=null,o=null;function fe(){if(!o)throw new Error("geen snapshot");const e=W(o.items);return Re(o.vector,o.stage,Pe(o.events,e),b())}function M(e){if(!o)throw new Error("geen snapshot");const t=b(),n=W(o.items),s=oe(o.events,e,n?.id),i=e.a===null?null:ne(e.a,s);return{done:se(s,t),plus:re(s,t),skip:ie(s,t),logged:ae(s,t),current:i}}function v(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}function S(){if(!o||!k)return;const e=fe(),{vector:t,stage:n}=o,s=k.mode==="local"?"Lokaal":"Supabase",i=`
    <div class="hdr">
      <div>
        ${ot()}
    ...
    <nav class="nav">
    ...
    </nav>`;if(d.screen==="vandaag"){const a=_e(o.profile.identity_new,o.events),c=He(o.items,b());J().innerHTML=`
      ${i}
      ${k.mode==="local"?'<div class="banner">Lokaal — geen Supabase. +1 / Done / Skip blijven op dit apparaat.</div>':""}
      ...
      ${d.error?`<p class="error" style="padding:0 18px">${v(d.error)}</p>`:""}
      ${r}`;return}}function ft(e,t,n){const s=M(e),i=C(e),r=s.logged||!!s.skip,a=i&&s.current!==null&&e.b!==null&&s.current>=e.b,c=r||a,f=i,$=Ke(e),l=f&&t.suggestedMilestone&&e.id===o.vector.id;return`
      <div class="card">
        ...
        <div class="actions ${i?"":"actions-two"}">
          ${i?`<button class="btn ico-btn ${s.plus?"on":""}" data-act="plus" data-item="${e.id}" ${c?"disabled":""}>${y("plus")}<span>+1</span></button>`:""}
        ...
        ${d.skipItemId===e.id||s.skip?`<div class="chips">${Ue.map(u=>`<button class="chip ${s.skip===u?"on":""}" data-act="skip" data-item="${e.id}" data-reason="${u}">${u}</button>`).join("")}</div>`:""}
        ...
        ${f&&n?`<div class="note">${v(n)}</div>`:""}
      </div>`}function g(e){return Number.isInteger(e)?String(e):e.toFixed(1)}async function P(e){if(!d.busy){d.busy=!0,d.error=null;try{await e()}catch(t){d.error=t instanceof Error?t.message:"Er ging iets mis"}finally{d.busy=!1,S()}}}async function pt(e){k=e,o=await k.load(),d.screen="vandaag",S()}async function vt(){ut(),yt(),await pt(rt())}function yt(){document.addEventListener("click",e=>{const t=e.target.closest("[data-act], [data-nav]");if(!t)return;const n=t.dataset.nav;if(n==="vandaag"||n==="koers"){d.screen=n,d.skipItemId=null,d.advanceWarn=!1,S();return}mt(t)})}async function mt(e){const t=e.dataset.act;if(!t||!k||!o)return;const n=fe(),s=b();if(t==="sleep-inc"||t==="sleep-dec"){const i=n.sleep??7,r=Math.max(0,Math.min(14,i+(t==="sleep-inc"?.5:-.5)));await B({date:s,kind:"body_sleep",value:r,skip_reason:null,item_id:null});return}if(t==="energy"){const i=Number(e.dataset.n),r=n.energy===i?null:i;if(r===null)return;await B({date:s,kind:"body_energy",value:r,skip_reason:null,item_id:null});return}if(t==="plus"){const i=o.items.find(a=>a.id===e.dataset.item);if(!i||!C(i))return;const r=M(i);if(r.logged||r.skip||i.b!==null&&r.current!==null&&r.current>=i.b)return;await B({date:s,kind:"set",value:(r.current??i.a??0)+1,skip_reason:null,item_id:i.id});return}if(t==="done"){const i=o.items.find(a=>a.id===e.dataset.item);if(!i)return;const r=M(i);if(r.logged||r.skip)return;await B({date:s,kind:"done",value:r.current??i.a,skip_reason:null,item_id:i.id});return}if(t==="skip-open"){const i=e.dataset.item??null;d.skipItemId=d.skipItemId===i?null:i,S();return}if(t==="skip"){const i=o.items.find(c=>c.id===e.dataset.item);if(!i||M(i).logged)return;const a=e.dataset.reason;if(!a)return;await B({date:s,kind:"skip",value:null,skip_reason:a,item_id:i.id}),d.skipItemId=null;return}if(t==="advance"){if(!n.suggestedMilestone)return;if(we(o.profile.identity_constraint)&&!d.advanceWarn){d.advanceWarn=!0,S();return}await Z(n.suggestedMilestone);return}if(t==="advance-go"){if(!n.suggestedMilestone)return;await Z(n.suggestedMilestone);return}if(t==="advance-cancel"){d.advanceWarn=!1,S();return}if(t==="save-ik"){const i={...o.profile,identity_anti:w(A("identity_anti"),m.identity_anti),identity_new:w(A("identity_new"),m.identity_new),identity_constraint:w(A("identity_constraint"),m.identity_constraint),horizon_1y:w(A("horizon_1y"),m.horizon_1y)};await P(async()=>{await k.saveProfile(i),o.profile=i});return}if(t==="export"){ge(o);return}}function A(e){return document.querySelector(`[data-id="${e}"]`)?.value??null}async function Z(e){await P(async()=>{o.stage=await k.advanceStage(o.stage,e),o.items=o.items.map(t=>t.id===o.stage.vector_id?{...t,milestone:e}:t),o.rotated=!0,d.advanceWarn=!1})}async function B(e){await P(async()=>{const t=await k.addEvent(e);o.events.push(t)})}vt();
